...
//!
//! This is what bare `cargo run -p warp_tui` builds, so it uses a local-only
//! channel config and needs no internal `warp-channel-config` generator
...
fn main() -> Result<()> {
    let mut state = ChannelState::new(
        Channel::Oss,
        ChannelConfig::local_only(AppId::new("dev", "warp", "WarpTui"), "warp-tui.log"),
    );
